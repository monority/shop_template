...
const Nav = () => {
	...
	const nav =  ["Brands", "Promo", "New Arrivals" , "Selection"];
	const [active, setActive] = useState("Brands");
	const display_nav = nav.map(item => {
		return (
			
			<li key={item} className={`${active === item ? 'active' : 'unactive'} element text_size01`}
			onClick={() => setActive(item)}><p>{item}</p></li>
		)
	})
	return (
		<>
			<nav className='h_100'>
				<div className="container_between">
					<ul className="wrapper_default_row padding_default">
					{display_nav}
					</ul>
